...
void OptionsImpl::validate() const {
  ...
  if (!backendEndpoints().empty()) {
    EndpointType first_endpoint_type = Utility::endpointTypeFromHostPort(backendEndpoints()[0]);
    for (const std::string& backend_endpoint : backendEndpoints()) {
      EndpointType endpoint_type = Utility::endpointTypeFromHostPort(backend_endpoint);
      if (endpoint_type == EndpointType::INVALID) {
      ...
      if (endpoint_type != first_endpoint_type) {
